...
export const Medical_appointments = () => {
  ...
  const handleSchedule = (id: string) => {
    ...
    // Aquí puedes almacenar el ID en el estado o en AsyncStorage si es necesario
  };
    ...
    <View style={{ flex: 1 }}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        {!calendarOpen ? (
          <>
            {arr.map((i, index) => (
              <View key={`${i.hospital}-${index}`} style={{ padding: 20 }}>
                <Reminder
                  title="Recordatorio de cita"
                  body="No olvides tu cita médica."
                  hours={parseInt(i.time)} // Por ejemplo, para las 2 PM
                  minutes={parseInt(i.minutes)} // Para los 30 minutos
                  onSchedule={handleSchedule} // Función para manejar el ID de la notificación
                />
                <DateTimeCita
                  Hospital={i.hospital}
                  Estado={i.estado}
              ...
              {hourOptions.map((hour, index) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.hourButton,
                    selectedHour === hour
                      ? styles.hourButtonActive
                      : styles.hourButtonInactive,
                  ]}
                >
                  <Text
                    style={[
...
const styles = StyleSheet.create({
  ...
  disabledButton: {
    // Estilo de confirmar cuando nadie lo toca
